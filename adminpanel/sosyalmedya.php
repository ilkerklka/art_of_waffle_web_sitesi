...
?>
        ...
        <form action="" method="post">
            ...
            if (isset($_POST["$id"])) {
                ...
                $urunekle = $baglan->prepare("UPDATE  sosyalmedya set
                link       =:kategori 
                where
                id =:id");
                ...
                if ($insert) {
                    echo '<div class="alert alert-success text-center">' .$urun["sosyalad"] .' başarıyla güncellendi . Site yenileniyor...</div>';

                    echo '<meta http-equiv="refresh" content="2;URL=sosyalmedya.php">';
                }
            }
            ?>
            <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label"><?php echo $urun["sosyalad"]?></label>
                <input type="text" name="kategori" class="form-control" id="Sifre" required>

            </div>
            <input type="hidden" name="<?php echo $id?>">
            <button style="border: black 1px solid;" type="submit" class="btn ">Güncelle</button>
            <small><b>AKTİF LİNK : </b> <?php echo $urun['link']?></small>
            <hr>
        </form>
